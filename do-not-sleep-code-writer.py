...
def type_into_notepad(content): #filename):
    ...
    # How many letters need to be typed before the mouse is moved slightly
    type_char = 0
    while True:
        for char in content:
            if not is_notepad_running():
                ...
                break
            type_char += 1
            # prevent sleeping or the screensaver
            prevent_sleep()
            if type_char == random.uniform(10,50):
                for move in movement_pattern:
                    pyautogui.moveRel(move[0],move[1], duration=0.01)
                type_char = 0
            if char.lower() in VK_CODES:
                vk_code = VK_CODES[str(char)]
                send_key(vk_code)
